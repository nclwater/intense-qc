...
'''
    ...
    df = pd.DataFrame(
        dict(start = np.flatnonzero((s.data.prev_qc_daily_acc == 0) & (s.data.QC_daily_accumualtions >= 1) & (s.data.next_qc_daily_acc >= 1)),
             end = np.flatnonzero((s.data.prev_qc_daily_acc >= 1) & (s.data.QC_daily_accumualtions >= 1) & (s.data.next_qc_daily_acc == 0))))
    ...
    for i in range(1,s.data.shape[0]-1):
        if rx1_to_exclude.iloc[i] == True and rx1_to_exclude.iloc[i-1] == False : # Will only change previous flags if state changes from F -> T
